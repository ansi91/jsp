...
//			}).then(res => res.text()).then(res => console.log(res));
			}).then(e => e.text()).then(function(text){ //이순신
				//alert("text: "+text);
				if("비밀번호가 틀립니다." ==text.trim()){//비밀번호가 틀립니다.
					...
					c_name = text.trim();
					//$.cookie('c_name',c_name); 		//클라이언트에서 결정 java script
				   ...
				   google.maps.event.addListener(marker, 'click',(function(marker,i){
					...
					//undefine XXXX오류 메시지 출력
					   return function() {//함수의 결과는 윈도우창을 열어줌.(click된 마커마다 다르게)
						   ...
						   infowindow.setContent(content +  '<b>' + jsonDoc[i].res_name+'</b><br><img src='+jsonDoc[i].res_photo+' width="200px" height="150px" >');
